...
export function SonifyStageMesh() {
  ...
  const aiResultImage = useKonvaImage(generation.resultUrl)
  ...
  const useAiResult = Boolean(generation.active && generation.resultUrl && aiResultImage && !compareHidden)

  useEffect(() => {
    if (useAiResult || !sourceFaceUrl || !sourceConfirmed || !layerReady || imageWidth <= 0 || imageHeight <= 0) return
    ...
    void renderFaceLayer(sourceFaceUrl, sourceCrop, sourceMask, sonFace.distortCorners, width, height)
      ...
      .catch(() => undefined)
  }, [cornersKey, imageHeight, imageWidth, layerReady, setWarpedPreview, sourceConfirmed, sourceCrop, sourceFaceUrl, sourceMask, sonFace.distortCorners, sonFace.height, sonFace.warpRevision, sonFace.width, useAiResult])
