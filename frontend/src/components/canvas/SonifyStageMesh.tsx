...
export function SonifyStageMesh() {
  ...
  const updateMeshPoint = useEditorStore((state) => state.updateMeshPoint)
  ...
  const meshKey = JSON.stringify(sonFace.mesh)
  const maskKey = JSON.stringify(sonFace.mask)

  useEffect(() => {
    if (!sourceFaceUrl || !layerRect || imageWidth <= 0 || imageHeight <= 0) return
    const timer = window.setTimeout(() => {
      ...
      void requestWarpPreview({
        ...
        updateSonFace({ warpedPreviewUrl: url })
      }).catch(() => undefined)
    }, 120)
    return () => window.clearTimeout(timer)
  }, [imageHeight, imageWidth, layerRect, maskKey, meshKey, sourceFaceUrl, sonFace.feather, sonFace.height, sonFace.mask, sonFace.mesh, sonFace.width, updateSonFace])
  ...
  const meshStagePoints = sonFace.mesh.map((point) => imageToStagePoint({ x: sonFace.x + point.targetX * sonFace.width, y: sonFace.y + point.targetY * sonFace.height }, imageRect, viewport))
  ...
  const meshLineRows = [0, 1, 2, 3].map((row) => meshStagePoints.slice(row * 4, row * 4 + 4).flatMap((point) => [point.x, point.y]))
  const meshLineColumns = [0, 1, 2, 3].map((column) => [0, 1, 2, 3].map((row) => meshStagePoints[row * 4 + column]).flatMap((point) => [point.x, point.y]))
  const maskLine = maskStagePoints.flatMap((point) => [point.x, point.y])

  useEffect(() => {
    if (activeTool !== 'manual-region') setManualRegion(null)
    const onKeyDown = (event: KeyboardEvent) => {
      if (event.key === 'Escape' && (faceEditMode === 'warp' || faceEditMode === 'mask')) setActiveTool('select')
      if (event.key === 'Delete' && selectedLayer === 'face' && !(event.target instanceof HTMLInputElement)) selectLayer(null)
    }
    window.addEventListener('keydown', onKeyDown)
    return () => window.removeEventListener('keydown', onKeyDown)
  }, [activeTool, faceEditMode, selectLayer, selectedLayer, setActiveTool])
      ...
      >
        ...
        <Layer name="replacementLayer">
          {warpedFaceImage && layerRect ? <KonvaImage
            ...
            opacity={sonFace.opacity}
            draggable={faceEditMode === 'move' && selectedLayer === 'face'}
            onClick={() => selectLayer('face')}
            onTap={() => selectLayer('face')}
            onDragEnd={(event) => updateSonFace({ x: stageToImagePoint({ x: event.target.x(), y: event.target.y() }, imageRect, viewport).x, y: stageToImagePoint({ x: event.target.x(), y: event.target.y() }, imageRect, viewport).y })}
            onTransformEnd={(event) => {
              ...
              updateSonFace({ x: nextPosition.x, y: nextPosition.y, width: nextWidth, height: nextHeight, rotation: node.rotation(), warpedPreviewUrl: null })
              node.scaleX(1)
              node.scaleY(1)
            }}
          /> : null}
        ...
        <Layer name="guideLayer">
          ...
          {faceEditMode === 'warp' && selectedLayer === 'face' ? <>
            {meshLineRows.map((points, index) => <Line key={`row-${index}`} points={points} stroke="#ffd166" strokeWidth={1.5} listening={false} />)}
            {meshLineColumns.map((points, index) => <Line key={`column-${index}`} points={points} stroke="#ffd166" strokeWidth={1.5} listening={false} />)}
            {meshStagePoints.map((point, index) => <Circle key={`mesh-${index}`} x={point.x} y={point.y} radius={8} fill="#ffd166" stroke="#161616" strokeWidth={2} draggable onDragMove={(event) => { const local = stageToLayerPoint({ x: event.target.x(), y: event.target.y() }); updateMeshPoint(index, { targetX: local.x, targetY: local.y }) }} />)}
          </> : null}
          {faceEditMode === 'mask' && selectedLayer === 'face' ? <>
            <Line points={maskLine} closed stroke="#ff8f70" strokeWidth={2} dash={[7, 4]} listening={false} />
            {maskStagePoints.map((point, index) => <Circle key={`mask-${index}`} x={point.x} y={point.y} radius={8} fill="#ff8f70" stroke="#161616" strokeWidth={2} draggable onDragMove={(event) => updateMaskPoint(index, stageToLayerPoint({ x: event.target.x(), y: event.target.y() }))} />)}
          </> : null}
        </Layer>
