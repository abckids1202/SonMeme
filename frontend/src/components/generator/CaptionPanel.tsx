...
export function CaptionPanel() {
  ...
  return <div className="caption-panel">
    ...
    <div className="caption-options"><label className="field"><span>Size</span><input type="range" min="0.65" max="1.5" step="0.05" value={caption.scale} onChange={(event) => updateCaption({ scale: Number(event.target.value) })} /></label><label className="field"><span>Angle</span><input type="range" min="-12" max="12" step="1" value={caption.rotation} onChange={(event) => updateCaption({ rotation: Number(event.target.value) })} /></label></div>
  </div>
}
